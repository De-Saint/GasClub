...
export class UserOrders {
  ...
  userid: any;
  noprod: any;
  constructor(public storage: Storage, public loadingCtrl: LoadingController, public navCtrl: NavController, public datalink: Datalink, public navParams: NavParams) {
    ...
    this.storage.ready().then(() => {
      this.storage.get('loggedInUserDetails').then((loggedInUserDetails) => {
        if (loggedInUserDetails == null) {
          ...
          loading.present();
          this.datalink.getPlacedOrders(this.userid, "Pending", "User")
            .subscribe(result => {
    ...
    this.storage.ready().then(() => {
      this.storage.get('loggedInUserDetails').then((loggedInUserDetails) => {
        if (loggedInUserDetails == null) {
          ...
          this.userid = this.UDetails.userid;
          loading.present();
          this.datalink.getPlacedOrders(this.userid, "Delivered", "User")
            .subscribe(result => {
              loading.dismiss().catch(() => { });
              if (result[0] === "200") {
                this.originalorders = result[1];
    ...
    this.storage.ready().then(() => {
      this.storage.get('loggedInUserDetails').then((loggedInUserDetails) => {
        if (loggedInUserDetails == null) {
          ...
          this.userid = this.UDetails.userid;
          loading.present();
          this.datalink.getPlacedOrders(this.userid, "Cancelled", "User")
            .subscribe(result => {
              loading.dismiss().catch(() => { });
              if (result[0] === "200") {
                this.originalorders = result[1];
  ...
  GotoOrderDetails(orderid) {
    this.navCtrl.push(UserOrderDetails, { orderid });
  }
